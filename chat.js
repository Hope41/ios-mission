...
function say(key, man = 'none', oneOff = true) {
    ...
    if (!spoken[key] || !oneOff) {
        ...
        if (key == 'introControls') {
            chat.say(hero, [.35, .55, .25, 0], ['Use the left and right arrow keys (A and D) to move.'], true)
            return
        }

        if (key == 'uptojump') {
            chat.say(hero, [.35, .55, .25, 0], ['Press the up key (W) to jump.'], true)
            return
        }

        if (key == 'introEnd') {
            chat.say(hero, [.35, .55, .25, 0], ['Press the down key (S) to interact with signs, doors or people.'], true)
            return
        }
...
class Chat {
    ...
    say(speaker, color, speech, passing = false) {
        this.active = true

        if (!passing) game.pause = true
        ...
        for (let i = 0; i < speech.length; i ++)
            this.queue.push({color: dic.color, speaker: dic.speaker, speech: speech[i], passing})
    ...
    update() {
        ...
        // stop if it says it's a passing comment
        if (this.queue[0].passing) {
            if (this.fade != 'out') {
                if (this.queue[1] && this.queue[1].speech == this.queue[0].speech)
                    this.newChat(false)
                else this.newChat()
            }
        }
